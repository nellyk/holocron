...
import {
  REGISTRY_MODULES_KEY, REGISTRY_MODULE_MAP_KEY, REGISTRY_MODULE_BLOCKED_KEY, MODULES_STORE_KEY,
...
export function createRegistryActions(state, dispatch) {
  return {
    getModuleMetaData(moduleName) {
      return this.getModuleMap().getIn([MODULES_STORE_KEY, moduleName]);
    },
...
export default function useModuleRegistry(
  ...
  return React.useMemo(() => ({
    ...
    dispatch,
    modules: state.get(REGISTRY_MODULES_KEY),
